...
def extract_hog(gray, 
                resize=(128,128),           # 고정 크기로 맞춰야 벡터 길이가 일정
                pixels_per_cell=(16,16), 
                cells_per_block=(2,2), 
                orientations=9):
    gray_r = cv2.resize(gray, resize)
    fd, _  = hog(gray_r,
...
def extract_features(path):
    ...
    f_hog = extract_hog(sharp) # eq로도 해보기

    # 최종 피처 결합: 174d + 100d = 274d
    return np.hstack([f_lbp_sh, f_laws, f_edge, f_sift, f_color, f_hog])
...
X_train_p=pca.fit_transform(X_train_fs)
X_test_p =pca.transform(X_test_fs)
